-- Select all our bike trip records for Mar 2024
  
  SELECT *
  FROM [Cyclistic].[dbo].[202403-cyclistic-tripdata];
...
SELECT COUNT(*)
  ...
  WHERE (started_at is NULL) OR (ended_at is NULL) OR (start_station_name is NULL) OR (end_station_name is NULL) OR (start_station_id is NULL) OR (end_station_id is NULL) OR (start_lat is NULL) OR (start_lng is NULL) OR (end_lat is NULL) OR (end_lng is NULL);

    -- Delete and remove the null rows from our dataset
  
  DELETE FROM [Cyclistic].[dbo].[202403-cyclistic-tripdata]
  ...
  sp_rename '[Cyclistic].[dbo].[202403-cyclistic-tripdata].ride_id', 'trip_id', 'COLUMN';
  sp_rename '[Cyclistic].[dbo].[202403-cyclistic-tripdata].rideable_type', 'bike_type', 'COLUMN';
  sp_rename '[Cyclistic].[dbo].[202403-cyclistic-tripdata].started_at', 'start_time', 'COLUMN';
  sp_rename '[Cyclistic].[dbo].[202403-cyclistic-tripdata].ended_at', 'end_time', 'COLUMN';
  sp_rename '[Cyclistic].[dbo].[202403-cyclistic-tripdata].member_casual', 'user_type', 'COLUMN';

   -- Add two new columns, as per our stakeholder instruction

  -- Add a ride length column as TIME

  ALTER TABLE [Cyclistic].[dbo].[202403-cyclistic-tripdata]
  ...
  UPDATE [Cyclistic].[dbo].[202403-cyclistic-tripdata]
